...
- **Audit log 接受 caller-supplied tenantId** ✅ §11.56：`apps/web-server/src/enterprise/auth-audit.ts` 中 `audit:log` / `audit:query` 两个 handler 现在接可选 `tenantId`（之前 schema 一直在，handler 却不接，等于把多租户能力埋在 store 里）。同时增加防御性类型校验：非 string 的 `tenantId` 抛 `INVALID_ARGUMENT` 而不是悄悄落到 'default'。`apps/web-server/src/common/audit-log.ts` 的 `QueryAuditFilters` 同步加 `tenantId?: string` 字段，查询时精确匹配；`''`（空串）视作 'default' tenant（隐式租户的便捷写法）。测试 +6（`apps/web-server/tests/audit-log-tenant.test.ts`，6 个 case）：默认 'default' / caller-supplied 落到正确租户 / 空串 → 'default' / 磁盘 round-trip / 无 tenantId filter 返全量 / 未知 tenantId 返空（无 false-positive default 回退）。tsc --noEmit 零新增错误；audit 套件 3 文件 / 20 测试全过。`§A.6` 基线 876 → 882（+6）。
