/**
 * ai/skills-sheet — Sheet-specific AI skills (formula suggest / explain / analyze / trend / chart / data-fill).
 */

import { registerHandle } from '../common/registry.js'

export function registerAiSkillsSheetHandlers(): void {
  registerHandle('ai:sheets-formula-suggest', async (_event: unknown, args: unknown) => {
    ...
    return {
      formulas: [
        { formula: '=SUM(A1:A10)', description: '求和公式，计算 A1 到 A10 的总和', score: 0.95 },
        { formula: '=AVERAGE(A1:A10)', description: '平均值公式，计算 A1 到 A10 的平均值', score: 0.90 },
        { formula: '=IF(A1>100,"高","低")', description: '条件判断，根据值返回不同结果', score: 0.85 },
      ],
      recommended: 0,
      range: dataRange,
    }
  ...
  registerHandle('ai:sheets-data-analyze', async (_event: unknown, args: unknown) => {
    ...
    return {
      ...
      recommendations: ['建议添加趋势线', '考虑过滤异常值', '可以创建数据透视表'],
    }
  })
