...
async function invoke(base: string, channel: string, args: unknown[]): Promise<{ status: number; body: unknown }> {
...
describe.skipIf(!haveBundle)('file-management IPC handlers', () => {
  ...
  it('home:properties returns stat + sha256', async () => {
    ...
    expect(r.status).toBe(200)
    const props = (r.body as { result?: { path?: string; sizeBytes?: number; hash?: string | null } })
      ?.result
  ...
  describe('Phase 3 — sheets routed through SheetsStore', () => {    it('sheets:new-blank allocates a file through SheetsStore.create', async () => {
...
describe.skipIf(!haveBundle)('file-management IPC handlers — atomic writes + dual-write P1-2', () => {
  let server: ChildProcess | undefined
  let base: string
  let dataDir: string
  ...
  it('web:save-file writes atomically and produces no tmp residue on success', async () => {
    ...
    expect(r.status).toBe(200)
    const result = r.body as { result?: { ok?: boolean; id?: string; path?: string; name?: string } }
    ...
    const tmpFiles = require('node:fs').readdirSync(dir).filter((n: string) => n.includes('.tmp'))
  ...
  it('web:save-file refuses zero-byte uploads', async () => {
    const r = await invoke(base, 'web:save-file', [{ name: 'empty.txt', bytes: new ArrayBuffer(0) }])
  ...
  it('docs:save updates both the legacy DOCS_RECENT mirror and unifiedRecents on disk', async () => {
    ...
    const list = ((recents.body as { result?: { entries?: Array<{ path: string }> } }).result?.entries ?? [])
  ...
  it('home:rename-file rejects unsafe newName inputs', async () => {
    ...
    for (const [bad, label] of evil) {
      ...
      expect.soft(body?.result?.error, `${label}: expected error message`).toMatch(/invalid file name|outside/)
...
describe.skipIf(!haveBundle)('file-management IPC handlers — recents file watcher (B4)', () => {
  ...
  it('removes a file from home:recents when it is deleted from disk', async () => {
    ...
    const beforePaths = (((before.body as { result?: { entries?: Array<{ path: string }> } })?.result?.entries) ?? []).map(
      (e) => e.path,
    )
    ...
    const afterPaths = (((after.body as { result?: { entries?: Array<{ path: string }> } })?.result?.entries) ?? []).map(
      (e) => e.path,
    )
...
describe.skipIf(!haveBundle)('file-management IPC handlers — preview thumbnails (B19)', () => {
  ...
  it('returns a base64 PNG data URL for a real PNG when sharp is available', async () => {
    ...
    expect(r.status).toBe(200)
    const body = r.body as { result?: { ok?: boolean; mime?: string; dataUrl?: string; size?: number; reason?: string } }
