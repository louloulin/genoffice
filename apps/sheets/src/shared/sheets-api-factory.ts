/// Transport-agnostic construction of the sheets renderer bridge APIs.
///
/// One source of truth for the `window.desktopApi` / `window.projectApi`
/// surface: the sandboxed preload builds it on an ipcRenderer transport
/// (Electron), the browser web-bridge builds the exact same object on the
/// HTTP/SSE transport (web version). Channel names, argument shapes, listener
/// wrappers, return coercion and validation are identical — only the transport
/// differs.
import type {
  AiChatResponse,
  AiSettings,
...
export interface SheetsApiOverrides {
  /** Web-native workbook open (browser file picker → temp file → open-path). */
  selectWorkbook?: () => Promise<WorkbookFile | null>
  /** Web-native merge selection (browser file picker → temp files → open-workbooks-for-merge). */
  selectWorkbooksForMerge?: () => Promise<WorkbookFile[] | null>
  /** Web-native CSV save confirm (browser download). */
